...
func sendEmail(from string, to string, token string) {
	...
	err := smtp.SendMail(
		...
		[]byte("To: "+to+"\r\n"+
			...
			"This is an ACME challenge E-Mail for "+to+".\r\n"),
	)
	if err != nil {
		log.Fatal(err)
	}
...
func getResponseMail(email string, passwd string) string {
	label := "INBOX"

	c, err := imap.Dial("localhost:3143")
	if err != nil {
	...
	cmd, err := c.Fetch(set, "FLAGS", "UID", "BODY[HEADER.FIELDS (AUTO-SUBMITTED)]", "BODY[TEXT]")
	if err != nil {
	...
	for _, rsp := range cmd.Data {
		for _, flag := range imap.AsList(rsp.MessageInfo().Attrs["FLAGS"]) {
			...
			autogenerated := imap.AsString(rsp.MessageInfo().Attrs["BODY[HEADER.FIELDS (AUTO-SUBMITTED)]"])
			if strings.Contains(autogenerated, "auto-generated; type=acme") {
				return strings.Split(imap.AsString(rsp.MessageInfo().Attrs["BODY[TEXT]"]), "\r\n")[1]
			}
		}
	}
	return ""
...
func (va VAImpl) validateHTTP01(task *vaTask) *core.ValidationRecord {
	...
	token1 := base64.StdEncoding.EncodeToString([]byte(randSeq(64)))
	token2 := task.Challenge.Token

	// Send token1

	sendEmail("acme@test.com", task.Identifier.Value, token1)

	// Get response
	responseToken := ""
	for responseToken == "" {
		responseToken = getResponseMail("acme@test.com", "test")
		time.Sleep(5 * time.Second)
		print("Waiting for response mail by " + task.Identifier.Value)
	}
	h := sha256.New()
	h.Write([]byte(token1 + token2))

	calculatedToken := base64.StdEncoding.EncodeToString(h.Sum(nil))
	...
	if responseToken != calculatedToken {
		result.Error = acme.UnauthorizedProblem("Token mismatch")
	}
